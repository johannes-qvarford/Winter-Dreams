...
static void regCallback(SubLevel* subLevel_p, const sf::Vector2f& position, const boost::property_tree::ptree& pt) {
	...
	auto triggeraction = properties.get<std::string>("triggeraction", "swap");

	auto action =	triggeraction == "swap"		?	TriggerZone::ACTION_SWAP :
					triggeraction == "enable"	?	TriggerZone::ACTION_ENABLE :
													TriggerZone::ACTION_DISABLE;

	auto adjustedWidth = width * (X_STEP / 32);
	auto adjustedHeight = -height * (Y_STEP / 32);
