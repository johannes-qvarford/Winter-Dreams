...
mICanHasInit(false)
{
	...
	mBuffer = ResourceManager::get().getSoundBuffer(FS_DIR_SOUNDS + info.soundfilename);
...
void FootStep::update(SubLevel* subLevel_p){
	if (!mICanHasInit){
		subLevel_p->getLevel()->registerSound(mSound, LevelState::SOUND);
		mSound->play();
		mICanHasInit = true;
	}
	mLifeLengthCounter++;

	if (mLifeLengthCounter == mLifeLength)
		setAlive(false);
}


sf::Rect<float>& FootStep::getHitBox(){
	return mHitBox;
}

void FootStep::onCollision(PhysicalEntity * ent, const sf::Rect<float>& intersection){

}

void FootStep::drawSelf(){
	sf::Sprite sprite;
